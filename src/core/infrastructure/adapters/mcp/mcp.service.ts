...
Issue tracking is enabled for this session.`,
    ...
    try {
      ...
      // Store agencyId in Redis for fast lookups (from IP-based auth)
      if (agencyId) {
        await this.redisService.set(
          `session:${finalSessionId}:agencyId`,
          agencyId,
          3600,
        );
      }
  ...
  private registerTools(server: McpServer) {
    ...
    server.tool(
      ...
      async ({ agents, components, skills, persona, dryRun }) => {
        try {
          const result = { dryRun, agents, components: [] as string[], skills: [] as string[], persona, errors: ['Install service not available - services were removed'], message: '⚠️ Ecosystem install service is not available (services were removed).' };
    ...
    server.tool(
      ...
      async ({ agents, components }) => {
        try {
          const result = { dryRun: false, agents, components: [] as string[], skills: [], errors: ['Sync service not available - services were removed'], message: '⚠️ Ecosystem sync service is not available (services were removed).' };
