...
export class SessionCleanupService {
  ...
  async performCleanup(): Promise<void> {
    ...
    try {
      ...
      if (expiredSessions.length > 0) {
        this.logger.log(
          `🗑️ DELETED ${expiredSessions.length} expired session(s)`,
        );
      }

      // 2. Expire unvalidated sessions older than 5 minutes
      const unvalidatedThreshold = new Date(
        now.getTime() - this.UNVALIDATED_SESSION_TIMEOUT_MS,
      );
      const unvalidatedSessions =
        await this.sessionRepository.findUnvalidatedSessionsOlderThan(
          unvalidatedThreshold,
        );
      ...
      if (unvalidatedSessions.length > 0) {
        this.logger.log(
          `⏰ EXPIRED ${unvalidatedSessions.length} unvalidated session(s)`,
        );
      }

      // 3. Expire inactive sessions (no activity for 24 hours)
      const inactiveThreshold = new Date(
        now.getTime() - this.INACTIVE_SESSION_TIMEOUT_MS,
      );
      const inactiveSessions =
        await this.sessionRepository.findInactiveSessionsSince(
          inactiveThreshold,
        );
      ...
      if (inactiveSessions.length > 0) {
        this.logger.log(
          `⏰ EXPIRED ${inactiveSessions.length} inactive session(s)`,
        );
      }
      ...
      this.logger.log(
        `📊 Stats: ${stats.totalSessions} total, ${stats.activeSessions} active, ${stats.expiredSessions || 0} expired, ${stats.totalMessages} messages`,
      );
      this.logger.log(
        `🧹 Cleanup completed. Processed ${cleanedCount} session(s)`,
      );
    } catch (error) {
  ...
  async forceCleanup(): Promise<{
    ...
    const inactiveSessions =
      await this.sessionRepository.findInactiveSessionsSince(inactiveThreshold);
    for (const session of inactiveSessions) {
      await this.sessionRepository.expire(session.sessionId);
      result.inactive++;
    }
