...
export class VectorStoreFactory {
  ...
  async create(config: VectorStoreFactoryConfig): Promise<IVectorStore> {
    ...
    if (this.stores.has(storeKey)) {
      this.logger.debug(`♻️ [VectorStoreFactory] Reusing existing ${config.type} store`);
      return this.stores.get(storeKey)!;
    }

    this.logger.log(`🏭 [VectorStoreFactory] Creating ${config.type} vector store...`);
    ...
    try {
      ...
      this.logger.log(`✅ [VectorStoreFactory] ${config.type} store connected successfully`);
      return store;
    } catch (error) {
      this.logger.error(`❌ [VectorStoreFactory] Failed to connect: ${error instanceof Error ? error.message : error}`);
      throw error;
    }
  }
