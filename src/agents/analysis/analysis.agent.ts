...
export class AnalysisAgent extends BaseAgent {
   ...
   */
  private checkRuleViolation(request: string, rule: any): { detected: boolean; reason: string } {
    ...
    if (rule.id.includes('di-') || rule.id.includes('dependency')) {
      if (requestLower.includes('new ') && requestLower.includes('service')) {
        return {
          detected: true,
          reason: 'Posible acoplamiento fuerte - considera usar inyección de dependencias',
        };
      }
    }
    ...
    if (rule.id.includes('clean') || rule.id.includes('layers')) {
      if (requestLower.includes('controller') && requestLower.includes('database')) {
        return {
          detected: true,
          reason: 'El controller no debería acceder directamente a la base de datos',
        };
      }
    }
    ...
    if (rule.id.includes('repository')) {
      if (requestLower.includes('find') && requestLower.includes('controller')) {
        return {
          detected: true,
          reason: 'El controller debería usar un repositorio, no acceder directamente',
        };
      }
    }
